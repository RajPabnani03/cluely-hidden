/**
 * The 6 builtin prompt profiles (ported verbatim from cheating-daddy).
 *
 * Source: https://github.com/sohzm/cheating-daddy — src/utils/prompts.js
 *
 * Each profile has 5 sections that compose into a single system prompt:
 *   intro + formatRequirements + (searchUsage?) + content + outputInstructions
 *
 * Text is ported unchanged — same wording, same examples. Do not paraphrase.
 */

export interface PromptProfileParts {
  intro: string;
...
/** The 6 builtin profiles with their full prompt templates (verbatim port). */
export const BUILTIN_PROFILES: Record<ProfileId, PromptProfileParts> = {
  interview: {
...
/** All builtin profile IDs in display order. */
export const ALL_PROFILE_IDS: ProfileId[] = [
  "interview",
  "sales",
  "meeting",
  "presentation",
...
/**
 * Build the full system prompt from a profile + custom context.
 * Mirrors cheating-daddy's `buildSystemPrompt(promptParts, customPrompt, googleSearchEnabled)`.
 *
 * Layout:
 *   {intro}\n\n{formatRequirements}
 *   [+\n\n{searchUsage} if googleSearchEnabled]
 *   \n\n{content}
...
export function buildSystemPrompt(
  ...
  const parts = BUILTIN_PROFILES[profile] || BUILTIN_PROFILES.interview;
  const sections: string[] = [parts.intro, "\n\n", parts.formatRequirements];
  ...
  sections.push(
    ...
    customPrompt,
    "\n-----\n\n",
    parts.outputInstructions,
  );
  return sections.join("");
}

/**
 * On app start, ensure the 6 builtin profiles have their full system prompts
 * in the DB. Safe to run multiple times — only updates if the DB prompt is empty.
 *
 * The Rust side seeds the 6 builtin rows with empty `system_prompt` on first
 * DB open. This helper back-fills the prompt text on every launch.
 */
export async function syncBuiltinProfilesToDb(): Promise<void> {
  try {
    // Lazy import to avoid a cycle / TSC dep explosion in non-IPC contexts
    const { listProfiles, updateProfile } = await import("./tauri");
    const dbProfiles = await listProfiles();
    for (const profileId of ALL_PROFILE_IDS) {
      const builtin = BUILTIN_PROFILES[profileId];
      if (!builtin) continue;
      const dbProfile = dbProfiles.find((p) => p.id === profileId);
      if (
        dbProfile &&
        (!dbProfile.system_prompt || dbProfile.system_prompt.trim() === "")
      ) {
        const fullPrompt = buildSystemPrompt(profileId, "", true);
        await updateProfile(profileId, undefined, fullPrompt);
      }
    }
  } catch (err) {
    console.error("syncBuiltinProfilesToDb failed:", err);
  }
}
